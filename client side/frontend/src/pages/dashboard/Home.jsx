// client side/frontend/src/pages/dashboard/Home.jsx

import React, { useState, useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import MultiLevelNavbar from '../../components/layouts/inputs/navBarCategories';
// *** Import the AnnonceCard component ***
import AnnonceCard from '../../components/layouts/inputs/annonceCard'; // Verify this path is correct
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchAnnonces = async () => {
      ...
      try {
        console.log("Fetching annonces from relative path:", API_PATHS.ANNONCE.ADD_GET_ANNONCE);
        ...
        if (err.response) {
          setError(`Error ${err.response.status}: ${err.response.data?.message || 'Failed to load announcements.'}`);
    ...
    <div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6 pb-4 border-b border-gray-200">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800"> Listings</h1>
          <Link
            to="/postad"
            className="inline-flex items-center bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-md shadow transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
          ...
          <>
            {annonces.length === 0 ? (
              ...
              <div className="text-center p-10 text-gray-500">
                ...
                <p>Be the first to <Link to="/postad" className="text-teal-700 hover:underline font-medium">post an ad</Link>!</p>
              </div>
            ) : (
